$(document).ready(function() {
	...
	los valores de esas cookies en cada campo correspondiente decodificando en Base 64 */
		...
		$("input#correo").val(Base64.decode(getCookie("correo")));
	...
	$("#recordar").change(function() {
		...
		if ($(this).is(":checked")) {
			...
			setCookie("correo", Base64.encode($("input#correo").val()));
	...
	$("input[value='Cancelar']").click(function() {
	...
	$("input[value='Valorar']").click(function() {

		$("form#valoracion input.identificador").val($(this).parent().parent().find("input.identificador").val());
		$("form#valoracion input.identificadorHotel").val($(this).parent().parent().find("input.identificadorHotel").val());
	...
	switch(nameElement) {
		...
		case "nombre":
			...
			if (elemento.val().length == 0) {
				...
				$(elemento).after("<span class = 'aviso'>Introduzca un nombre</span>");
				mostrarSpan();
				bandera = false;
		...
		case "apellidos":
			...
			if (elemento.val().length == 0) {
				...
				$(elemento).after("<span class = 'aviso'>Introduzca apellidos</span>");
				mostrarSpan();
				bandera = false;
				...
				if (elemento.val().length > 30) {

					elemento.val(elemento.val().substr(0, 30));
		...
		case "password":
			...
			if (elemento.val().length == 0) {
				...
				$(elemento).after("<span class = 'aviso'>Introduzca una contraseña</span>");
				mostrarSpan();
				bandera = false;
		...
		case "asunto":
			...
			if (elemento.val().length == 0) {
				...
				$(elemento).after("<span class = 'aviso'>Introduzca un asunto</span>");
				mostrarSpan();
				bandera = false;
